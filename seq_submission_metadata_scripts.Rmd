...
```{bash, "download_nanopore", echo=TRUE, eval=FALSE}
# pull fastq files for Nanopore data - need to be concatted by barcode.  Script also checks if the files are gzipped and compresses them if they are not
for run in "$@"; do
	echo $run
	mkdir -p ${run}_fastq
	cd ${run}_fastq
...
8. Select 'String of N's represents expected missing sequence against a reference'
9. Depending on where we are against historic data either select 'yes' - Genbank will remove these from your submission and release the rest.  The final Rscript will pull the metadata for the rejected samples to add to the 'ONGOING_submissions" tab".  If you select 'no' then you will go back and forth with GenBank with coverage information etc.
