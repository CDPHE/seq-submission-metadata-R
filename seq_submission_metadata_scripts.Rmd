...
9. Aspera Connect Ascp: gsutil cp -r gs://laura-sandbox/aspera-connect/ . Then you can fin ascp in aspera/connect/bin
...
Rscript gisaid_ncbi_biosample_metadata_formatting.R -m /home/mandy_waters/test_metadata -f test_fasta.fa -s mandywaters -t "Oxford Nanopore GridION" -r /home/mandy_waters/rerun_samples_11Sep2021.tsv
...
# You need to type Rscript  before your Rscript to run (Rscript gisaid_ncbi_biosample_metadata_formatting.R)
Rscript gisaid_ncbi_biosample_metadata_formatting.R --metadata /home/mandy_waters/test_metadata -f test_fasta.fa --submitter_name mandywaters --seq_type "Oxford Nanopore GridION" --rerun_file /home/mandy_waters/rerun_samples_11Sep2021.tsv

# Strings with spaces need quotes
```

**Output to terminal:**
Script prints the strings the user input for you to check.  If you don't privde values for all flags then the script will error out and tell you which flag(s) are missing.  The script will also error out if the sequence type name is misspelled.  A warning will print that lists the accession name, coverage, and project number for samples that are missing a collection date.  It will not error out, but will print this info and write it to a file for users to review.
...
7. CONDITIONAL: samples to delete file is written if you have the originally sequenced files in your batch of projects (samples_to_delete_2021-09-11.tsv)
...
Rscript gisaid_ncbi_biosample_metadata_formatting.R -m ~/home/mandy_waters -f test.fa -s mandy.waters -t "Oxford Nanopore GridION" 
